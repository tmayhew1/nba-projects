library(tidyverse); library(httr); library(XML); library(rvest); library(ggplot2); library(ggthemes); library(plotly); library(gridExtra); library(DT); library(scales); library(shinyWidgets); library(shiny);library(scales)
...
## Single Season
p1i = psearch("Jay Huff")
s1i = "2024-2025"
p2i = psearch("Myles Turner")
s2i = "2015-2016"
player_df = df %>% filter(Player == p1i,Year==s1i) %>% rbind.data.frame(df %>% filter(Player == p2i,Year==s2i))
top_c = player_df$Hex[1]
to_p = player_df %>% transmute(Player = paste0(Player," (",Year,")"),`2-Pointers Added`=2*(X2PAdd)/G,`3-Pointers Added`=3*(X3PAdd)/G,`Free Throws Added`=FTAdd/G,`Value Added`=valueAdd/G,`Scoring Volume Added`=vaPTSv/G,`Assists Added`=vaAST/G,`Steals Added`=vaSTL/G,`Blocks Added`=vaBLK/G,`Turnovers Added`=vaTOV/G, `Defensive Rebounds Added`=vaDRB/G, `Offensive Rebounds Added`=vaORB/G) %>% gather("key","value",-Player)
to_p %>% ggplot(aes(x = key,y = value,fill = Player)) + geom_bar(stat="identity",color="black",position = "dodge") + coord_flip() + theme_bw() + geom_hline(yintercept = 0) + scale_y_continuous(trans = trans_new(name = "signed_sqrt",transform = function(x) sign(x) * sqrt(abs(x)),inverse = function(x) sign(x) * (x^2)),breaks = seq(-100,100,1))

## Peak Calculation
seasons_i = sample(x = c("1","2","3","4","5"),1)