...
# Inputs
player_input_1 <- psearch("Austin Reaves")
player_input_2 <- psearch("Luka D")
...
# Normalize player names (strip trailing "(...)" in both data and inputs)
tp_df <- tp_df %>%
  mutate(Player_dp = str_replace(Player, "\\s*\\(.*\\)$", ""))

player_input_1_dp <- str_replace(player_input_1, "\\s*\\(.*\\)$", "")
player_input_2_dp <- str_replace(player_input_2, "\\s*\\(.*\\)$", "")

# Extract top_color for input 1 (safe fallback if not found)
top_color <- tp_df %>%
  filter(Player_dp == player_input_1_dp) %>%
  pull(Hex) %>%
  { if (length(.) >= 1) .[1] else "#2C7FB8" }   # fallback hex

# Factor ordering (use normalized names)
tp_df$Player <- factor(tp_df$Player, levels = unique(tp_df$Player))
...
# Display string
tp_df <- tp_df %>%
  mutate(disPlayer = paste0(
    Year_dp, " ", Player_dp, ": ",
    sprintf("%.2f", (PTS/G)), " PTS on ",
    eFG., " eFG%"
  )) %>%
  arrange(Player_dp)

# Color group keyed on normalized player names
tp_df <- tp_df %>%
  mutate(color_group = case_when(
    Player_dp == player_input_1_dp ~ "input1",
    Player_dp == player_input_2_dp ~ "input2",
    TRUE ~ NA_character_
  ))

# Data for labels + vertical nudge
label_df <- tp_df %>%
  filter((Player_dp == player_input_1_dp & Year == year_input_1) |
           (Player_dp == player_input_2_dp & Year == year_input_2)) %>%
  mutate(y_adj = ifelse(PTSAdd/G == max(PTSAdd/G), (PTSAdd/G) + 0.25, (PTSAdd/G) - 0.25))

# Plot 1 of 4 - Scoring Volume vs. Scoring Efficiency
tp_df %>%
  ggplot(aes(x = vaPTSv/G, y = PTSAdd/G)) +
  geom_smooth(se = FALSE, method = "lm", formula = 'y ~ x',
              color = "grey70", alpha = 0.75, inherit.aes = FALSE) +
  geom_point(aes(color = color_group), alpha = 0.6, na.rm = TRUE) +
  scale_y_continuous(
  ...
  geom_text(
    data = label_df,
    aes(x = vaPTSv/G, y = y_adj, label = disPlayer, color = color_group),
    size = 3
  ) +
  scale_color_manual(
    values = c("input1" = top_color, "input2" = "grey50"),
    na.value = "grey80",
    drop = FALSE
  )