...
pi1 = psearch("Tim Duncan")#"Michael Jordan (jordami01)"
pi2 = psearch("Charles Barkley")#"LeBron James (jamesle01)"
...
p_static = p_static %>% separate(Player,into = c("disPlayer","bbref"),sep = " \\(",remove = F) %>% mutate(Rk = "")
...
for (i in 1:nrow(p_static)){
  ...
  p_static$Rk[i] = which(temp2$Player == p_static$Player[i])
}
p_static = p_static %>% mutate(Rk = as.integer(Rk)) %>% arrange(Rk,desc(Stat))
p_static$alpha = 1-percent_rank(p_static$Rk)
p_static = p_static %>% inner_join(df %>% transmute(Player,Year, PTS/G, TRB/G, AST/G, `STK/G` = (STL+BLK)/G, `FG%`=100*FG., `3P%`=100*X3P., `FT%`=100*FT.) %>% 
  ...
  select(-c(Player, bbref, gpl, Rk, Stat))
...
p_static = p_static %>% mutate(across(c(`PTS/G`, `TRB/G`, `AST/G`, `STK/G`), ~ sprintf("%.1f", .))) %>%
  ...
  select(-G) %>% unite(col = "PTS | TRB | AST | STK", c(`PTS/G`,`TRB/G`,`AST/G`,`STK/G`),remove = T, sep = " | ") %>% 
  unite(col = "FG% | 3P% | FT%", c(`FG%`,`3P%`,`FT%`),remove = T, sep = " | ")
p_static = p_static %>% select(`Value (Rank)`, Year, Team,`PTS | TRB | AST | STK`,`FG% | 3P% | FT%`, everything())
...
names(p_static)[4:14] = c(" FG | 3P | FT"," PTS | TRB | AST | STK"," Team"," Year",str_split(pi1," \\(")[[1]][1],
                          " ", #stat_input?
                          str_split(pi2," \\(")[[1]][1],rev(c("FG | 3P | FT ","PTS | TRB | AST | STK ","Team ","Year ")))

# names(p_static)[which(colnames(p_static) == "Value..Rank.")] = str_split(pi1," \\(")[[1]][1]
# names(p_static)[which(colnames(p_static) == "Value..Rank..1")] = str_split(pi2," \\(")[[1]][1]
